...
struct Vector3f : public nw4r::math::VEC3 {
	...
	friend Vector3f operator*(f32 f, const Vector3f& v) { return Vector3f(v.x * f, v.y * f, v.z * f); }
	...
	void mul(const Vector3f& v, const Vector3f& w) { set(v.x * w.x, v.y * w.y, v.z * w.z); }
	...
	Vector3f cross(const Vector3f& b) const DECOMP_DONT_INLINE
	{
		f32 _x = (y * b.z) - (z * b.y);
		f32 _y = (z * b.x) - (x * b.z);
		f32 _z = (x * b.y) - (y * b.x);
	...
	Vector3f negated() { return Vector3f(-x, -y, -z); }
	...
	bool isZero() const { return squaredLength() <= Math<f32>::epsilon(); }
	...
	// _00-_0C = nw4r::math::VEC3
};

/**
 * @brief Basic 2D vector.
 *
 * @note Size: 0x8.
 */
